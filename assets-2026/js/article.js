// VIB - Page article individuelle (refonte)
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function renderArticle(a) {
        ...
        // Canonical + Open Graph par article (URL propre /<slug>)
        const cleanUrl = `https://village-iraty-biarritz.fr/${a.slug}`;
        const setMeta = (sel, attr, val) => { const el = document.querySelector(sel); if (el) el.setAttribute(attr, val); };
        setMeta('link[rel="canonical"]', 'href', cleanUrl);
        setMeta('meta[property="og:url"]', 'content', cleanUrl);
        setMeta('meta[property="og:title"]', 'content', `${a.title} - VILLAGE Iraty-Biarritz`);
        setMeta('meta[property="og:description"]', 'content', desc);
        if (a.image) {
            setMeta('meta[property="og:image"]', 'content', `https://village-iraty-biarritz.fr${a.image}`);
        }
